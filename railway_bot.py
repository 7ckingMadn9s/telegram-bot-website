...
Система:
- Главный админ: может удалять ЛЮБЫХ админов, видит список всех админов
- Обычные админы: могут только отправлять тестовые заявки
НЕТ кнопок: "помощь", "удалить себя", "список администраторов" для обычных админов
"""
        ...
        # ========== ТЕСТОВАЯ ЗАЯВКА (ДЛЯ ВСЕХ АДМИНОВ) ==========
            ...
            if user_is_admin:
                ...
                sent, total = send_to_admins(
                    ...
                    exclude_id=user.id  # Не отправляем себе
                )
                
                bot.edit_message_text(
                    chat_id=call.message.chat.id,
                    message_id=call.message.message_id,
                    text=f"✅ Тестовая заявка отправлена {sent} админам из {total}!",
                    parse_mode='HTML'
                )
            else:
                bot.edit_message_text(
...
if __name__ == '__main__':
    ...
    print("\n" + "=" * 50)
    print("🤖 БОТ С ГЛАВНЫМ АДМИНИСТРАТОРОМ")
    print("=" * 50)
